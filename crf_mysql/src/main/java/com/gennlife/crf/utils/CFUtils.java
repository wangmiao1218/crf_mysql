...
/**
 * @Description: 基于用户的协同过滤推荐算法实现
 * @author: wangmiao
 * @Date: 2018年7月1日 下午3:42:09 
 */
public class CFUtils {
	...
	public static void main(String[] args) {
		...
		Map<String, Integer> userItemLength = new HashMap<>();// 存储每一个用户对应的不同物品总数
																// eg: A 3
		Map<String, Set<String>> itemUserCollection = new HashMap<>();// 建立物品到用户的倒排表
																		// eg: a
																		// A B
		...
		// 计算相似度矩阵【稀疏】
		Set<Entry<String, Set<String>>> entrySet = itemUserCollection
				.entrySet();
		...
		for (int j = 0; j < sparseMatrix.length; j++) {
			if (j != recommendUserId) {
				System.out.println(idUser.get(recommendUserId)
						...
						/ Math.sqrt(userItemLength.get(idUser
								.get(recommendUserId))
		...
		for (String item : items) {// 遍历每一件物品
			...
			if (!users.contains(recommendUser)) {// 如果被推荐用户没有购买当前物品，则进行推荐度计算
				...
				for (String user : users) {
					itemRecommendDegree += sparseMatrix[userID
							.get(recommendUser)][userID.get(user)]
